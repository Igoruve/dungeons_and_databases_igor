...
-- -----------------------------------------------------
-- Table `dungeons_and_databases`.`feat`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `dungeons_and_databases`.`feat` ;

CREATE TABLE IF NOT EXISTS `dungeons_and_databases`.`feat` (
...
-- -----------------------------------------------------
-- Table `dungeons_and_databases`.`background`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `dungeons_and_databases`.`background` ;

CREATE TABLE IF NOT EXISTS `dungeons_and_databases`.`background` (
  `background_id` INT NOT NULL AUTO_INCREMENT,
  `feat_id` INT NOT NULL,
  `skill_id` INT NOT NULL,
  `item_id` INT UNSIGNED NOT NULL,  -- Asegúrate de que sea UNSIGNED
  PRIMARY KEY (`background_id`),
  INDEX `fk_background_feat1_idx` (`feat_id` ASC) VISIBLE,
  INDEX `fk_background_skill1_idx` (`skill_id` ASC) VISIBLE,
  INDEX `fk_background_item1_idx` (`item_id` ASC) VISIBLE,
  CONSTRAINT `fk_background_feat`
    FOREIGN KEY (`feat_id`)
    REFERENCES `dungeons_and_databases`.`feat` (`feat_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_background_skill`
    FOREIGN KEY (`skill_id`)
    REFERENCES `dungeons_and_databases`.`skill` (`skill_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_background_item`
    FOREIGN KEY (`item_id`)
...
CREATE TABLE IF NOT EXISTS `dungeons_and_databases`.`character` (
  ...
  INDEX `fk_character_background1_idx` (`background_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_character_background1`
    ...
    REFERENCES `dungeons_and_databases`.`background` (`background_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `dungeons_and_databases`.`user` (
  ...
  INDEX `fk_users_notes1_idx` (`note_id` ASC) VISIBLE,
  INDEX `fk_user_character1_idx` (`character_id` ASC) VISIBLE,
  CONSTRAINT `fk_users_notes1`
    FOREIGN KEY (`note_id`)
...
-- -----------------------------------------------------
-- Table `dungeons_and_databases`.`character_has_feat`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `dungeons_and_databases`.`character_has_feat` ;

CREATE TABLE IF NOT EXISTS `dungeons_and_databases`.`character_has_feat` (
  `character_id` INT UNSIGNED NOT NULL,
  `feat_id` INT NOT NULL,
  PRIMARY KEY (`character_id`, `feat_id`),
  INDEX `fk_character_has_feat_feat1_idx` (`feat_id` ASC) VISIBLE,
  INDEX `fk_character_has_feat_character1_idx` (`character_id` ASC) VISIBLE,
  CONSTRAINT `fk_character_has_feat_character1`
    FOREIGN KEY (`character_id`)
    REFERENCES `dungeons_and_databases`.`character` (`character_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_character_has_feat_feat1`
    FOREIGN KEY (`feat_id`)
    REFERENCES `dungeons_and_databases`.`feat` (`feat_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
(1, 'Alert', 'Always on the lookout for danger, you gain +5 to initiative, can''t be surprised while conscious, and other creatures don''t gain advantage on attack rolls against you as a result of being hidden from you.'),
(2, 'Dual Wielder', 'You master fighting with two weapons. You gain +1 AC while wielding a separate melee weapon in each hand. You can use two-weapon fighting even when the one-handed melee weapons aren''t light. You can draw or stow two one-handed weapons when you would normally be able to draw or stow only one.'),
(3, 'Lucky', 'You have inexplicable luck that seems to kick in at just the right moment. You have 3 luck points. Whenever you make an attack roll, an ability check, or a saving throw, you can spend one luck point to roll an additional d20.'),
(4, 'War Caster', 'You have practiced casting spells in the midst of combat. You have advantage on Constitution saving throws that you make to maintain your concentration on a spell when you take damage. You can perform the somatic components of spells even when you have weapons or a shield in one or both hands.'),
(5, 'Mobile', 'You are exceptionally speedy and agile. Your speed increases by 10 feet. When you use the Dash action, difficult terrain doesn''t cost you extra movement on that turn. When you make a melee attack against a creature, you don''t provoke opportunity attacks from that creature for the rest of the turn.'),
(6, 'Tough', 'Your hit point maximum increases by an amount equal to twice your level when you gain this feat. Whenever you gain a level thereafter, your hit point maximum increases by an additional 2 hit points.');
...
-- Background items, skills, feats relationships
INSERT INTO background (background_id, feat_id, skill_id, item_id) VALUES
(1, 1, 8, 17), -- Soldier
(2, 3, 16, 4), -- Criminal
(3, 4, 3, 11), -- Sage
(4, 6, 14, 13), -- Entertainer
(5, 5, 18, 12); -- Outlander
...
-- Character Feats
INSERT INTO character_has_feat (character_id, feat_id) VALUES
(1, 1), -- Thokk Level 1: Alert
(2, 3), -- Lilith Level 1: Lucky
(3, 4), -- Alaric Level 1: War Caster
(4, 6), -- Gareth Level 1: Tough
(5, 3), -- Melody Level 1: Lucky
(6, 1), -- Thokk Level 5: Alert
(6, 2), -- Thokk Level 5: Dual Wielder
(7, 3), -- Lilith Level 5: Lucky
(7, 5), -- Lilith Level 5: Mobile
(8, 4), -- Alaric Level 5: War Caster
(8, 6), -- Alaric Level 5: Tough
(9, 6), -- Gareth Level 5: Tough
(9, 4), -- Gareth Level 5: War Caster
(10, 5), -- Bramble Level 5: Mobile
(10, 6); -- Bramble Level 5: Tough
...
INSERT INTO `dungeons_and_databases`.`user` 
(`user_id`, `email`, `password`, `created_at`, `first_name`, `last_name`, `role`, `nickname`, `note_id`, `character_id`) 
VALUES 
(1, 'dungeon.master@gmail.com', 'password123', '2025-03-15 18:30:00', 'Morgan', 'Blackwood', 'master', 'ArchMage', NULL, NULL),
(2, 'barbarian.player@gmail.com', 'axeswing456', '2025-03-16 10:15:00', 'Mike', 'Johnson', 'player', 'MountainCrusher', NULL, 1),
(3, 'rogue.player@gmail.com', 'sneaky789', '2025-03-16 14:20:00', 'Sarah', 'Williams', 'player', 'ShadowDancer', NULL, 2),
(4, 'wizard.player@gmail.com', 'fireball123', '2025-03-17 09:45:00', 'Thomas', 'Chen', 'player', 'SpellSeeker', NULL, 3),
(5, 'druid.player@gmail.com', 'naturelover777', '2025-03-18 11:30:00', 'Emma', 'Garcia', 'player', 'WildShape', NULL, 10);

-- Associate master user with notes
INSERT INTO `dungeons_and_databases`.`user_has_notes` 
(`user_id`, `notes_id`) 
VALUES 
(1, 1),
(1, 2),
